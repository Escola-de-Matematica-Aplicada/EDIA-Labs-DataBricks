...
# ─────────────────────────────────────────────────────────────
# GABARITO DAS ANOMALIAS — para uso exclusivo do professor
# ─────────────────────────────────────────────────────────────

print("""
╔══════════════════════════════════════════════════════════════╗
║  GABARITO — Anomalias da Parte 4 (não mostrar aos alunos)   ║
╠══════════════════════════════════════════════════════════════╣
║                                                              ║
║  1. DUPLICATAS (~100 linhas)                                 ║
║     Detecção:                                                ║
║       SELECT pedido_id, data_entrega, COUNT(*)               ║
║       FROM fato_entregas                                     ║
║       GROUP BY 1,2 HAVING COUNT(*) > 1;                      ║
║                                                              ║
║  2. VALORES NEGATIVOS (~50 linhas)                           ║
║     Detecção:                                                ║
║       SELECT * FROM fato_entregas                            ║
║       WHERE distancia_km < 0                                 ║
║          OR peso_carga_kg < 0                                ║
║          OR custo_combustivel < 0;                           ║
║                                                              ║
║  3. OUTLIERS atraso_min (~30 linhas)                         ║
║     Detecção:                                                ║
║       SELECT * FROM fato_entregas                            ║
║       WHERE atraso_min > 1000 OR atraso_min < -480;          ║
║                                                              ║
║  4. FK QUEBRADA — sk_cliente 201–300 (~50 linhas)            ║
║     Detecção:                                                ║
║       SELECT fe.sk_cliente, COUNT(*) AS orfaos               ║
║       FROM fato_entregas fe                                  ║
║       LEFT JOIN dim_cliente dc USING (sk_cliente)            ║
║       WHERE dc.sk_cliente IS NULL                            ║
